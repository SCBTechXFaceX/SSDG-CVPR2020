...
from utils.get_loader import get_kaggle_dataset
...
def train():
    ...
    tgt_valid_dataloader = get_kaggle_dataset(config.src1_data, config.src1_train_num_frames, 
    ...
    print("learn all image at iter: ", all_image_iter // config.batch_size)
    ...
    for iter_num in range(max_iter+1):
        ...
        real_domain_label_3 = torch.LongTensor(input3_real_shape, 1).fill_(0).to(device)
        ...
        fake_domain_label_3 = torch.LongTensor(input3_fake_shape, 1).fill_(3).to(device)
